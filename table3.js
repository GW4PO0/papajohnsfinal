document.addEventListener("DOMContentLoaded", () => {
    ...
    const tables = document.querySelectorAll(".table1, .table2, .table3, .table4, .table5, .table6, .table8, .table9, .rectab1, .rectab2, .rectab3, .rectab4, .rectab5, .rectab6, .rectab7, .rectab8, .rectab9, .rectab10");
    ...
    const loadReservations = (date) => {
        tables.forEach((table, index) => {
            const isReserved = sessionStorage.getItem(`table3-${date}-table-${index}`);
            if (isReserved === "reserved") {
                table.style.backgroundColor = "#5d3919";
    ...
    const saveReservation = (date, index) => {
        sessionStorage.setItem(`table3-${date}-table-${index}`, "reserved");
    };
    ...
    confirmReservationButton.addEventListener("click", () => {
        selectedTable.style.backgroundColor = "#8d4c32;";
    ...
    finalOkButton.addEventListener("click", () => {
        ...
        window.location.href = "index.html"; // Redirect to final.html
    });
