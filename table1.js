document.addEventListener("DOMContentLoaded", () => {
    ...
    const tables = document.querySelectorAll(".table1, .table2, .table3, .table4, .table5, .table6, .table8, .table9, .rectab1, .rectab2, .rectab3, .rectab4, .rectab5, .rectab6, .rectab7");
    ...
    const loadReservations = (date) => {
        tables.forEach((table, index) => {
            const isReserved = sessionStorage.getItem(`table1-${date}-table-${index}`);
    ...
    const saveReservation = (date, index) => {
        sessionStorage.setItem(`table1-${date}-table-${index}`, "reserved");
    };
